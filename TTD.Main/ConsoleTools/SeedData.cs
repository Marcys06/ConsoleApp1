    ...
    public static class SeedData
    {
        public static void Execute(AppDbContext dbContext)
        {
            try
            {
                ...
                try
                {
                    // Próba odczytu – jeśli tabela nie istnieje, wyjątek zostanie złapany
                    tableExists = dbContext.Trains.Any();
                }
                catch
                {
                    tableExists = false;
                ...
                if (!tableExists)
                {
                    Console.WriteLine("   ⚠️ Tabela Trains nie istnieje.");
                    Console.WriteLine("   📝 Tworzenie bazy danych i tabel...");

                    // Wymuś utworzenie bazy danych
